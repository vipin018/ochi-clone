...
function Navbar() {
	...
	return (
		<div
			className={`transform transition-transform duration-700 ${show ? "translate-y-0" : "-translate-y-full"
				} fixed backdrop-blur-sm z-[999] w-full px-20 py-5 font-robotoCondensed flex justify-between items-center`}
		>
			<div className="logo">
				<svg
					width="72"
			...
			<div className="Links flex gap-10">
				{["Services", "Our Work", "About Us", "Insights", "Contact Us"].map(
					(item, index) => (
						<a
							key={index}
							className={`text-[25px] capitalize font-medium ${index === 4 && "ml-32"
								} transition-transform duration-300 transform hover:scale-110 hover:font-bold hover:text-[#004D43]`} // {{ edit_1 }}
						>
							{item}
						</a>
					)
